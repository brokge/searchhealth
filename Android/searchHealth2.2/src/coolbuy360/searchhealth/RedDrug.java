...
import coolbuy360.adapter.RedListViewAdapter;
...
public class RedDrug extends Activity {
	RedListViewAdapter listAdapter;
	private ListView red_drug_listview;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		red_drug_listview = (ListView) this.findViewById(R.id.Exposure_list);
		...
		red_drug_listview.setVisibility(View.GONE);
		// ××××××××××××××××××准备初始化foot_loading控件×××××××××××××××××××××
		...
		red_drug_listview.addFooterView(loadMoreView); // 设置列表底部视图
		
		new AsyncLoader_GuessInfo().execute();
	}

	private void initData(List<Map<String, String>> druglist) {
		listAdapter = new RedListViewAdapter(RedDrug.this, druglist);
		listAdapter.count = druglist.size();
		if (druglist.size() < pageSize) {
			loadMoreView.setVisibility(View.GONE);
		}
		red_drug_listview.setVisibility(View.VISIBLE);
		red_drug_listview.setAdapter(listAdapter);
		
		red_drug_listview.setOnItemClickListener(new itemOnClick());
		red_drug_listview.setOnScrollListener(new onScrollListener());
	...
	private final class itemOnClick implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int posion,
				long arg3) {
			// TODO Auto-generated method stub
			// Toast.makeText(RedDrug.this, "haha", 1).show();
			Map<String, String> map = (Map<String, String>) red_drug_listview
					.getItemAtPosition(posion);
			String drugId = map.get("redlistid");
			Bundle bundle = new Bundle();
			bundle.putString("drugnameid", drugId);
			bundle.putString("drugtype", "2");

			Intent drugintent = new Intent().setClass(RedDrug.this,
	...
	private class AsynLoader_more extends AsyncTask<String, Void, Integer> {
		...
		protected Integer doInBackground(String... params) {
			...
			try {
				innerdruglist = coolbuy360.logic.RedDrug.getList(pageSize,
						pageIndex + 1);
	...
	class AsyncLoader_GuessInfo extends AsyncTask<String, Void, Integer> {
		...
		protected Integer doInBackground(String... params) {
			try {
				// 加载数据
				innerdruglist = coolbuy360.logic.RedDrug.getList(pageSize, 1);
				if (innerdruglist != null) {
					return (innerdruglist.size() > 0) ? 0 : 1;
				} else {
					return 2;
		...
		protected void onPostExecute(Integer result) {
			if (result == 0) {
				...
				async_error_txt.setText("没有找到相关的红榜信息");
