...
import coolbuy360.adapter.MemberOperationListAdapter;
 ...
 *
 */
public class MemberOperation extends Activity implements OnItemClickListener {
	...
	private GestureListView member_operation_liv;	
	...
	private MemberOperationListAdapter adapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.member_operation);
		// 为退出做准备
		...
		actionbar_add_btn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				Intent intent = new Intent().setClass(MemberOperation.this, MemberOperationInfo.class);
	    		intent.putExtra("action", "add");
	    		startActivityForResult(intent, Action_Add);
			}
		});
		
		member_operation_liv = (GestureListView) this
				.findViewById(R.id.member_operation_liv);
		...
		member_operation_liv.addFooterView(loadMoreView);
		// *******************************载入的加载事件×××××××××××××××××××××××××××××××
		...
		member_operation_liv.setVisibility(View.GONE);
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int pos, long arg3) {
		// TODO Auto-generated method stub
		char flingState = member_operation_liv.getFlingState();
        Log.v("MY_TAG", "onItemClick: state="+flingState+", pos="+pos);

    	switch(flingState) {
    	// 处理左滑事件
    	case GestureListView.FLING_LEFT: {
    		member_operation_liv.setFlingState(GestureListView.FLING_CLICK);
        	break;
    	}
        // 处理右滑事件
    	...
    	case GestureListView.FLING_CLICK: {
    		...
    		Intent intent = new Intent().setClass(this,	MemberOperationInfo.class);
    		intent.putExtra("action", "edit");
    		intent.putExtra("position", pos);
    		intent.putExtra("id", item.get("id"));
    		intent.putExtra("implementtime", item.get("implementtime"));
    		intent.putExtra("operationname", item.get("operationname"));
	...
	private void initAdapter(List<Map<String, String>> sourceList) {
		// TODO Auto-generated method stub
		adapter = new MemberOperationListAdapter(MemberOperation.this, sourceList);
		if (sourceList.size() < pageSize) {
			loadMoreView.setVisibility(View.GONE);
		} else {
		}
		member_operation_liv.setVisibility(View.VISIBLE);
		member_operation_liv.setAdapter(adapter);
		...
		// 设置点击项事件
		member_operation_liv.setOnItemClickListener(this);

		member_operation_liv.setOnScrollListener(new OnScrollListener() {
			@Override
	...
	private class AsyncLoadList extends
		...
		protected Integer doInBackground(String... params) {
			try {
				...
				if (memberid != null) {
					innerlist = coolbuy360.logic.MemberOperation
		...
		protected void onPostExecute(Integer result) {
			if (result == 0) {
				...
				async_error_txt.setText("没有手术记录");
	...
	private class AsyncDelete extends AsyncTask<Integer, Void, Boolean> {
		...
		protected void onPreExecute() {
			// TODO Auto-generated method stub
			pBarcheck = new Dialog(MemberOperation.this, R.style.dialog);			
		...
		protected Boolean doInBackground(Integer... params) {
			...
			try {
				if (params.length > 0) {
					...
					CommandResult resultmessage = coolbuy360.logic.MemberOperation.Delete(id);
		...
		protected void onPostExecute(Boolean result) {
			...
			if (result)// 如果删除成功
			{
				member_operation_liv.deleteSuccess(position);
				pBarcheck.cancel();
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == RESULT_OK) { 
			...
			case Action_Edit:
				...
				Map<String, String> item = (Map<String, String>)adapter.getItem(position);
				item.put("implementtime", resultBundle.getString("implementtime"));
				item.put("operationname", resultBundle.getString("operationname"));
