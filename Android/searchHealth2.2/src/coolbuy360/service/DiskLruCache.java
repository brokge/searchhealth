 ...
 * source code (libcore/luni/src/main/java/libcore/io/DiskLruCache.java) and is preferable to this simple implementation.
 */
public class DiskLruCache {
	...
	/**
	 * 刷新缓存,删除老条目总的大小超过指定的缓存大小。注意,这不是跟踪过期文件的缓存目录
	 *不是在HashMap中。 如果图片和key在磁盘缓存经常改变然后他们可能永远不会被删除。
	 */
	 ...
	 * 创建一个缓存文件路径常量        使用当前的缓存目录和一个图像key
	 * @param key
	 * @return
	 */
	...
	/**
	 * 写入bitmap到一文件中 Call {@link DiskLruCache#setCompressParams(CompressFormat, int)} first to set the target bitmap compression and format.
	 * compress=压缩
	 * @param bitmap
	 * @param file
	 * @return
