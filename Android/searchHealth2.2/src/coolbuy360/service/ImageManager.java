...
public class ImageManager {
	...
	private static searchApp myapp;
	...
	/** 如果图片是从网络加载，则应用渐显动画，娀果从缓存读出则不应用动画 */
	...
	public static ImageManager from(Context context) {
		...
		if (myapp == null) {
			myapp = (searchApp) context.getApplicationContext();
		}
...
*/		
			...
			case MSG_REQUEST: // 收到请求
				...
				if (msg.obj != null && msg.obj instanceof ImageRef) {
					...
					/*	if (imageRef.width != 0 && imageRef.height != 0) {
							try {
								bitmap = android.media.ThumbnailUtils.extractThumbnail(tBitmap,
						...
						try {
							...
							if (data != null) {
								...
								int bitmapSize = opt.outHeight * opt.outWidth
										* 3;// pixels*3 if it's RGB and pixels*4
											// if it's ARGB
	...
	public String urlToFilePath(String url) {
		...
		// 图片文件名
		filePath.append(MD5.getMD5(url)).append(url.substring(index));
